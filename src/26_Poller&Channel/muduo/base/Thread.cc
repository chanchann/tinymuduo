...
pid_t gettid()
{
  //return static_cast<pid_t>(::syscall(SYS_gettid));   // ::syscall(SYS_gettid)总是返回-1，应该是MacOS不准许此操作
  return static_cast<pid_t>(::syscall(SYS_thread_selfid));    // 采用::syscall(SYS_thread_selfid)代替，获取到的tid位数不是6，是8.需要响应调整
}
