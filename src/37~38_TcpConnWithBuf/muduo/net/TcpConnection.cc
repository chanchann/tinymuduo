  ...
  */
  ...
  if (!error && remaining > 0)
  {
    ...
    if (oldLen + remaining >= highWaterMark_
        && oldLen < highWaterMark_     // 旧长度肯定未超过高水位
        && highWaterMarkCallback_)
    {
      loop_->queueInLoop(boost::bind(highWaterMarkCallback_, shared_from_this(), oldLen + remaining));
    }
    outputBuffer_.append(static_cast<const char*>(data)+nwrote, remaining);  // 未写完的数据添加到output buffer中
    if (!channel_->isWriting())
    {
      channel_->enableWriting();		// 关注POLLOUT事件，可写时将output buffer中的数据写到内核缓冲区
    }
  }
}
...
{
  loop_->assertInLoopThread();
  if (channel_->isWriting())   // 说明应用层Buffer中还有数据，否则不会关注POLLOUT事件
  {
