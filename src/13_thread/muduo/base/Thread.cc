...
void afterFork()
{
  ...
  muduo::CurrentThread::t_threadName = "main";  // 新创建的进程只有一个线程，自然就是主线程
  CurrentThread::tid();
...
class ThreadNameInitializer
{
 public:
  ThreadNameInitializer()
  {
    ...
    pthread_atfork(NULL, NULL, &afterFork);   // fork成功后，子进程会调用afterfork()
    
  }
};

ThreadNameInitializer init;       // 全局变量。在进入main()函数之前构造，即完成了主线程的tid，名称等的保存工作
}
}
...
{
  if (t_cachedTid == 0)  // 说明还未缓存过
  {
    t_cachedTid = detail::gettid();
    int n = snprintf(t_tidString, sizeof t_tidString, "%5d ", t_cachedTid);
    assert(n == 6);
    (void) n;  // 在Release版本中，assert语句被隐藏，所以会警告n未使用；此语句是为了消除该警告。  
  }
}
